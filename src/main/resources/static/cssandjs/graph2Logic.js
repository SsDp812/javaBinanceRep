anychart.onDocumentReady(function () {
    let pair = document.querySelector(".sele2").value;
    let bal1 = document.querySelector('.balanceNumber21');
    let bal2 = document.querySelector('.balanceNumber22');
    let Label =  document.querySelector(".sele2")
    let coinLabel = (Label.options[Label.selectedIndex].text)
    let coins = coinLabel.split('/')
    let title2 = ""

    document.querySelector(".sele2").addEventListener('change', function (e) {
        pair = e.target.value;
    ...
    .then(prices => {
        ...
        if(param!="first"){
            chart2.data(dataCh)
            title2= chart2.title(pair);
        }else{
            chart2.background().fill("black");
            series = chart2.box(dataCh);
            title2= chart2.title(pair);
            chart2.interactivity("by-x");
            chart2.container("chartWindow2");
            chart2.labels().fontColor("black");
            chart2.width('100%');
            chart2.height('100%');
            chart2.draw();
        }
    ...
    function getPriceForlabel(pair){
      ...
      .then(r => r.json())
      .then(price => {
        let label = document.querySelector('.priceNumber2');
    ...
    setInterval(function print(param) {
      ...
      let buyBT = document.querySelector('.b2buy');
      let sellBT = document.querySelector('.b2sell');
